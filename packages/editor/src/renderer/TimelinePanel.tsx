...
function childLabel(docChild: import("@seam/core").Child | undefined, resolved: ResolvedChild): string {
  if (docChild) {
    ...
    if (docChild.type === "html") {
      // HTML "source" is the markup itself — show a short, single-line preview
      // rather than the full string in the timeline block.
      const stripped = docChild.source.replace(/\s+/g, " ").trim();
      return stripped.length > 24 ? stripped.slice(0, 24) + "…" : stripped || "html";
    }
  ...
  if (resolved.type === "html") return "html";
