...
describe("buildFfmpegCommand", () => {
  it("builds a simple two-clip timeline", () => {
    ...
    // Second clip starts at t=3, so its PTS gets shifted forward
    expect(cmd.filterComplex).toContain("color=c=black@0:s=1920x1080:r=30:d=3");
    expect(cmd.filterComplex).toContain("concat=n=2:v=1:a=0");
  ...
  it("handles empty segments with color and anullsrc", () => {
    ...
    expect(cmd.filterComplex).toContain("color=c=black");
    expect(cmd.filterComplex).toContain("overlay=0:0:eof_action=pass");
    // Empty segment is skipped, second clip prefixed with 5s of transparent frames
    expect(cmd.filterComplex).toContain("color=c=black@0:s=1920x1080:r=30:d=5");
  });
  ...
  it("builds overlay with overlay filter", () => {
    ...
    expect(cmd.filterComplex).toContain("overlay=0:0:eof_action=pass");
    // Second child has delay=1s, prefixed with a 1s transparent color source
    expect(cmd.filterComplex).toContain("color=c=black@0:s=1920x1080:r=30:d=1");
