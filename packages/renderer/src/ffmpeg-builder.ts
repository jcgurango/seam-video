...
function buildCompositeSegment(
  ...
  // them and they're mixed straight into the audio output. Text segments
  // contribute no `a` label — they're silent by definition, and folding
  // dozens of `anullsrc` streams through `amix` was costing real wall
  // time at startup for nothing.
  const childSegments: {
    v?: string;
    a?: string;
    delay: number;
  ...
  for (const child of childSegments) {
    ...
    //
    // `tpad=start_duration=N` would do this in one filter, but it
    // eagerly materializes every padding frame: a 1920×1080 RGBA
    // overlay starting at t=75s buffers ~18 GB of black frames before
    // the real content, and with dozens of attachments the whole
    // graph deadlocks under memory pressure. `setpts=PTS+N/TB` is
    // cheap but leaves overlay blocked waiting for the secondary's
    // first frame.
    //
    // Instead: prepend a transparent `color` source (which produces
    // frames lazily, frame-by-frame) and `concat` it onto the front
    // of the child stream. Frames flow through one at a time —
    // bounded memory regardless of the delay length.
    if (child.delay > 0) {
      if (childV != null) {
        const blankV = `[blank${seg}]`;
        const childFmtV = `[cfmt${seg}]`;
        const delayedV = `[dv${seg}]`;
        ctx.filters.push(
          `color=c=black@0:s=${width}x${height}:r=${fps}:d=${child.delay},format=yuva420p,setsar=1${blankV}`
        );
        ctx.filters.push(
          `${childV}format=yuva420p,setsar=1${childFmtV}`
        );
        ctx.filters.push(
          `${blankV}${childFmtV}concat=n=2:v=1:a=0${delayedV}`
        );
        childV = delayedV;
      }
    ...
    if (childA != null) audioLabels.push(childA);
  }
 ...
 * segments return without a video label; silent segments (text) return
 * without an audio label so the composite mixer doesn't pay to fold in
 * dozens of empty `anullsrc` streams.
 */
 ...
 *  values use a literal; animated values compile straight from the
 *  keyframes using analytical expressions for native CSS easings (one
 *  expression per keyframe segment, no per-frame sampling) — see
 *  `keyframedToExpression`. Returns "" when volume is effectively
 *  unity so the filter chain stays minimal. */
function buildVolumeFilter(
  ...
  if (isStaticOrConstant(volume)) {
    const v = volume[0][1];
    return v === 1 ? "" : `,volume=${v}`;
  }
  return `,volume=eval=frame:volume='${keyframedToExpression(volume, duration, fps)}'`;
}
...
function buildTextSegment(
  ...
  // No silent `anullsrc` here — text contributes nothing to the audio
  // mix, and folding dozens of empty streams through `amix` was costly
  // at startup. The composite mixer treats a missing `a` as "skip".
  void parentW; // kept for signature symmetry with sibling builders
  void parentH;
  void dur;     // computed for the loop input; not used past the chain
...
function buildFilterChain(
  ...
  // Helper: turns a numeric Keyframed (or undefined) into a literal
  // "name=value" piece when static / effectively-constant, or a
  // "name='<expr>'" piece for genuinely animated values. The
  // expression is built from the keyframes directly (one analytical
  // expression per native-easing segment, no per-frame sampling
  // unless cubic-bezier), see `keyframedToExpression`.
  const pwlOf = (
    ...
    return {
      piece: `'${keyframedToExpression(value, duration, fps)}'`,
      animated: true,
    };
  };
  ...
  for (const f of filters) {
    switch (f.type) {
      ...
      case "opacity": {
        // colorchannelmixer doesn't support eval=frame for `aa`, so
        // animated values become a sendcmd source ahead of a labelled
        // instance. Static / effectively-constant values short-circuit
        // before sampling.
        if (!isKeyframed(f.value)) {
          body.push(`format=rgba,colorchannelmixer=aa=${f.value}`);
          break;
        }
        if (isStaticOrConstant(f.value)) {
          body.push(`format=rgba,colorchannelmixer=aa=${f.value[0][1]}`);
          break;
        }
      ...
      case "colorbalance": {
        ...
        // Only label the filter when a key is *genuinely* animated —
        // a keyframed-but-effectively-constant value stays a literal.
        const trulyAnimatedKeys = KEYS.filter((k) => {
          const v = f[k] as Keyframed<number> | undefined;
          return isKeyframed(v) && !isStaticOrConstant(v);
        });
        const id = trulyAnimatedKeys.length > 0 ? labelFor("cb") : null;
        const pieces: string[] = [];
        for (const k of KEYS) {
          const value = f[k] as Keyframed<number> | undefined;
          if (value == null) continue;
          if (!isKeyframed(value)) {
            if (value !== 0) pieces.push(`${k}=${value}`);
            continue;
          }
          if (isStaticOrConstant(value)) {
            const v = value[0][1];
            if (v !== 0) pieces.push(`${k}=${v}`);
            continue;
          }
      ...
      case "colortemperature": {
        ...
        if (isStaticOrConstant(f.temperature)) {
          body.push(`colortemperature=temperature=${f.temperature[0][1]}`);
          break;
        }
