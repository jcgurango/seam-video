...
describe("attachments", () => {
  ...
  describe("deep id lookup", () => {
    it("finds an id on a clip nested inside a child composition", () => {
      const result = resolveComposition(
        comp({
          children: [
            {
      ...
      // Inner composition: empty [0, 0.5] + myclip [0.5, 2.5] (local).
      // Outer: inner comp runs at speed 1 starting at t=0, so myclip sits
      // at outer [0.5, 2.5]. source time 1 on myclip (sourceIn=2, speed=1)
      // → outer = 0.5 + (1 - 2)/1 = -0.5.
      expect(result.children[1]).toMatchObject({
        source: "other.mp4",
        timelineStart: -0.5,
    ...
    it("throws on duplicate ids found in the deep walk", () => {
      expect(() =>
        resolveComposition(
          comp({
            children: [
    ...
    it("source-mode lookup through a nested clip's own speed", () => {
      const result = resolveComposition(
        comp({
          children: [
            {
      ...
      // target: inner_local [2, 5] (source 0-6 at 2x = 3s), outer matches
      // since inner comp speed = 1. source time 3 → outer = 2 + (3-0)/2 = 3.5
      expect(result.children[1]).toMatchObject({
        source: "x.mp4",
        timelineStart: 3.5,
        timelineEnd: 4.5,
