...
| `src/composition-prerender.ts` | `prerenderCompositionMlts()` — recursively renders each *complex* composition (own spatial / non-fit objectFit / filters) to a sidecar sub-`.mlt`, keyed by node identity; the builder references each as a producer placed at its display rect. Trivial comps still flatten |
