...
| `src/renderer/nodePath.ts` | **Path-keyed addressing** — `NodePath` (`{field,index}[]`), `pathKey`/`parsePath` (`children.3.attachments.1`), immutable tree `get`/`updateComp`/`remove`/`insert`/`moveNode` (cross-container; index-shift aware), `editContainer` lens (runs any `(Composition)→Composition` tool at a path, injecting the root bin for nested `binItem` resolution), and root-index adapters (`rootIndicesFromKeys`/`rootKeyFromIndex`) the legacy root-only tools consume |
| `src/renderer/dropRegions.ts` | `flattenDropRegions` — walks the layout tree into content-space drop targets (one per editable container), `regionAt`/`insertionIndexIn`/`insertionXIn`/`localTime`. Geometry invariant: `contentX / pxPerSec` is global output time at every nesting level (the window transform reconstructs it), so a container's local time at a cursor is `originSec + (contentX − containerLeft)/pxPerSec · scale` |
...
- **Path-keyed selection (single source of truth)**: `App` holds `selection: string[]` of path keys (`children.0`, `children.3.attachments.1` — see `nodePath.ts`). The UI layer resolves each key to a node. Nested **editable** blocks (regular comps, not `binItem`/`script` expansions — those carry `editable:false` on their `TreeGroup`) support select / resize / delete / drag at any depth; mutations run the existing pure tools (`resizeChild`, `attachNewItems`, …) against the target composition through the `editContainer` lens. The legacy root-only toolbar actions (attach/compose/bin/slice/word/transcribe in `ControlsBar`, anchor lines, CC-cut) consume **flat root indices** derived via `rootIndicesFromKeys` and ignore nested selections.
- **Unified timeline drag** (`DesktopTimeline` + `dropRegions.ts`): one drag pass handles reorder, drag **in/out of compositions**, and OS file-drop into any container. A grabbed block (`onReorderDragStart(path,…)`) or file is hit-tested against the flattened drop regions → target container + insertion slot → `moveNode` / `onImportAt`. The `[`/`]` **attach drop-zone** appears for a single selected clip/audio/composition primary (root **or** a nested clean — non-`in`/`out`-windowed — container, since `attachNewItems` resolves the container and a window would crop the 1:1 index map) and anchors the drop at the primary's container-local playhead. Resize deltas in a nested group scale by the group's window `scale`.
