...
| `src/audio.ts` | `renderAudioMix()` — `OfflineAudioContext` (`node-web-audio-api`) mix mirroring the timeline: per clip/audio node trim (`[in,out]` region decode) + speed (`playbackRate`, pitch-shifted) + gain envelope + crossfade ramps, summed at the destination. Runs **before** the frame loop (a graphic's jsdom globals later clobber `Event`, which would break OfflineAudioContext) |
