...
| `src/composition-prerender.ts` | `prerenderCompositionMlts()` — recursively renders each *complex* composition (own spatial / non-fit objectFit / filters / `backgroundColor` / **`inset`**) to a sidecar sub-`.mlt`, keyed by node identity; the builder references each as a producer placed at its display rect. Sub-`.mlt`s render on a **transparent** base (`defaultBackgroundColor: "#00000000"`) so they layer cleanly, with the comp's `speed` baked in via `rootSpeed` (stretched comps fill their window); trivial comps still flatten. **`inset` (crop):** the content renders to one sub-`.mlt`, then a *cropping* sub-`.mlt` (`buildCropNode`, fixed `cw×ch` canvas) affine-places it so the visible `sourceRect` zoom-fills the canvas (overflow clipped by the canvas); animated inset bakes per-frame size/translation keyframes → the builder's affine `rect` keyframe string. No crop filter, no varying canvas — animation rides the rect baker |
...
- **Spatial model**: every node lays out via `origin` + `translation` + `size` (no more `top/left/right/bottom/width/height/position`):
  ...
  - `inset` (+ `insetMode`) (`inset` feature, **composition-only**, animatable): per-edge crop in the text-padding shorthand but `Length`-valued (`%` → left/right=width, top/bottom=height). Order: objectFit/`size` build the content box → `origin`/`translation`/`size`/`rotation` place **that box** (placement decoupled from the crop — no drift) → `inset` clips a window + `SpatialRect.sourceRect` `{u0,v0,u1,v1}` records the visible sub-rect → `insetMode` (`window` default / `center` / `fit` / `cover`) maps the window within the placed box: window/center/fit change the output rect (`sourceRect` = the window); `cover` instead tightens `sourceRect` to a centered box-aspect crop of the window and outputs the full box (no second clip region needed — same visible result as objectFit cover, reusing the pipeline). `sampleInset` (core) reuses `sampleLength` per edge; **all of this lives in `resolveBoxProps`** — both renderers consume `SpatialRect` + `sourceRect` and need no mode-specific code (net scale stays uniform across all modes). Preview: a `crop: vec4f` uniform maps the quad into the sub-rect (comp FBO group; `sourceRect` forces the FBO path). MLT: see `composition-prerender.ts` (cropping sub-`.mlt`). To crop a clip/image, wrap it in a composition — the windowing cost is an authored opt-in.
